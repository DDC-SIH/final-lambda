...
from utils.base_metadata_handler import MetadataHandler
...
class INSAT3DProcessor:
    ...
    def process(self) -> Dict[str, str]:
        ...
        # Handle metadata first - Changed from upload_metadata to handle_metadata
        logger.info(f"Extracting metadata from {self.input_path}")
        if not self.metadata_handler.handle_metadata(self.input_path):
            logger.error("Failed to handle metadata")
